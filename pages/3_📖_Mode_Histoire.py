...
st.set_page_config(page_title="Mode Histoire", page_icon="📖", layout="wide")
ui.init_app_state()
profile = ui.require_profile()
ui.sidebar_switcher()
name = st.session_state.active_profile

st.title("📖 Mode Histoire")
st.caption("Une histoire à écrire ensemble. Ta sœur peut reprendre l'histoire là où tu l'as laissée !")

ai_mode = ai_client.is_configured()
with st.sidebar:
    st.markdown("---")
    st.success(f"🤖 Histoire IA adaptative ({ai_client.get_model()})") if ai_mode else \
        st.warning("📴 Mode chapitres fixes (aucune clé API)")

story_ids = list(cb.STORIES.keys())
labels = [f"{cb.STORIES[sid]['title']} ({cb.STORIES[sid]['cefr']})" for sid in story_ids]
choice = st.selectbox("Choisis une histoire :", options=list(range(len(story_ids))), format_func=lambda i: labels[i])
...
if st.button("🔄 Recommencer cette histoire"):
    _reset()
    st.rerun()
    ...
    if entry["role"] == "narrator":
        ...
        if entry.get("corrections"):
            with st.container(border=True):
                st.markdown("**✏️ Petite remarque amicale sur ta contribution précédente :**")
...
# ===========================================================================
if ai_mode:
    contribution = st.chat_input("Écris la suite en français...")
    if contribution:
        if not st.session_state[transcript_key]:
            st.session_state[transcript_key].append({"role": "narrator", "text": f"**Chapitre 1**\n\n{first_chapter['fr']}"})
        ...
        try:
            ...
            chapter_idx += 1
            next_text = f"**Chapitre {chapter_idx + 1}**\n\n{result['paragraph']}"
            ...
            game.mark_story_active_this_week(profile)
            game.award_xp(profile, 12, f"Histoire : {story['title']}")
            ...
            if result["story_ended"]:
                ...
                game.award_xp(profile, 20, f"Histoire terminée : {story['title']}")
            ...
            st.warning("L'IA n'a pas pu continuer l'histoire cette fois -- réessaie dans un instant.")
...
# ===========================================================================
    ...
    if chapter_idx > 0:
        st.markdown(f"**Chapitre {chapter_idx + 1}**")
    ...
    if chapter["prompt"]:
        ...
        contribution = st.chat_input("Écris la suite en français...")
        if contribution:
            st.session_state[transcript_key].append({"role": "narrator", "text": f"**Chapitre {chapter_idx + 1}**\n\n{chapter['fr']}"})
            st.session_state[transcript_key].append({"role": "user", "text": contribution})
            game.award_xp(profile, 12, f"Histoire : {story['title']}")
            ...
            newly = game.new_badges(profile)
            ui.save()
            st.toast("+12 XP ! La suite de l'histoire t'attend.", icon="📖")
            st.rerun()
    else:
        if st.button("Terminer l'histoire 🎉"):
            st.session_state[transcript_key].append({"role": "narrator", "text": f"**Chapitre {chapter_idx + 1}**\n\n{chapter['fr']}"})
            result = game.award_xp(profile, 20, f"Histoire terminée : {story['title']}")
            profile["story_progress"][story_id] = chapter_idx + 1
            game.mark_story_active_this_week(profile)
            if story_id not in profile["stories_completed"]:
                profile["stories_completed"].append(story_id)
