...
A daily-habit French immersion app for you (and anyone else you invite --
supports any number of profiles), built as a
...
│   ├── gamification.py             # XP, levels, badges, quest rotation
...
└── pages/
    ...
    ├── 3_📖_Story_Mode.py            # collaborative storytelling
    ├── 4_🧠_Vocab_Review.py         # spaced-repetition flashcards
    ├── 5_🎯_Quests.py               # daily/weekly quests + badge gallery
    ├── 6_🏆_Family_Leaderboard.py   # multi-profile leaderboard + progress charts
...
profile (name, starting CEFR level, avatar) -- and add more profiles any
time from the home page if you want someone else to use the app too --
then go to **⚙️ Settings** to add your Claude API key.

### Getting a Claude API key
1. Go to [console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys)
   and sign up / log in.
2. Create a new API key, copy it (starts with `sk-ant-...`).
3. In the app's **Settings** page, paste it in, pick a model, and either:
   - **"Use for this session"** -- kept in memory only, gone when
     you close the browser tab (safest, re-enter each time), or
   - **"Save on this device"** -- written to `data/ai_config.json`
...
4. You get a shareable URL like `https://your-app.streamlit.app`.
   ...
   - **Automatic (recommended):** add `GITHUB_TOKEN` + `GIST_ID` secrets so
     ...
     configured, the **⚙️ Settings** page shows "✅ Persistent backup
     enabled (GitHub Gist)".
   - **Manual:** periodically use **Settings → Download backup**
     to back up your progress, and **Restore** if the cloud instance
     ever resets.
...
- **More quests:** add to `QUESTS_DAILY` or `QUESTS_WEEKLY`.
...
- **Social accountability (optional):** if more than one person uses the
  app, the shared dashboard and leaderboard add friendly visibility into
  each other's progress, which can boost follow-through versus practicing
  alone -- but every profile's progress and quests are fully independent.
